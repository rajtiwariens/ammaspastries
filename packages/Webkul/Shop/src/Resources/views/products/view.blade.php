    ...
    @pushOnce('scripts')
        <script type="text/x-template" id="v-product-template">
            <x-shop::form v-slot="{ meta, errors, handleSubmit }" as="div">
                <form ref="formData" @submit="handleSubmit($event, addToCart)" enctype="multipart/form-data">
                                ...
                                @if ($product->sku == '006')
                                    <div class="mt-4">
                                        <div class="control-group">
                                            <label for="cake_name" class="text-base font-medium">Name on Cake</label>
                                            <input
                                                type="text"
                                                name="cake_name"
                                                id="cake_name"
                                                class="control w-full p-2 border rounded mt-1"
                                        ...
                                        <div class="control-group mt-4">
                                            <label for="cake_image" class="text-base font-medium">Upload Photo</label>
                                            <input
                                                type="file"
                                                name="cake_image"
                                                id="cake_image"
        ...
        <script type="module">
            app.component('v-product', {
                    ...
                    handleImageUpload(event) {
                        ...
                        if (file) {
                            if (!file.type.startsWith('image/')) {
                                this.$emitter.emit('add-flash', { type: 'warning', message: 'Please upload an image file' });
                                return;
                            }
                            if (file.size > 5 * 1024 * 1024) { // 5MB limit
                                this.$emitter.emit('add-flash', { type: 'warning', message: 'File size must be less than 5MB' });
                                return;
                            }
                ...
                mounted() {
                    const fileInput = this.$refs.formData.querySelector('#cake_image');
                    if (fileInput) {
                        fileInput.addEventListener('change', this.handleImageUpload.bind(this));
                    } else {
                        console.warn('File input #cake_image not found');
                    }
                },
