    ...
    public function store()
{
    $this->validate(request(), [
        ...
        'quantity'   => 'sometimes|integer|min:1', // Optional, defaults to 1 later
        'cake_name'  => 'nullable|string|max:255',
        'cake_image' => 'nullable|image|max:5120', // 5MB limit, image only
    ]);
    ...
    try {
        ...
        // Handle file upload for cake_image
        if (request()->hasFile('cake_image')) {
            $file = request()->file('cake_image');
            $path = $file->store('cake_images'); // Store in storage/app/public/cake_images
            $data['cake_image'] = Storage::url($path); // Add URL to additional data
        }

        // Add cake_name to additional data if present
        if (request()->has('cake_name')) {
            $data['cake_name'] = request()->input('cake_name');
        }
